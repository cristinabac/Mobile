...
class DatabaseHelper {
  ...
  String dbName = 'items4.db';


  void _createDb(Database db, int newVersion) async {
    await db.execute('CREATE TABLE $itemsTable($colIdLocal INTEGER PRIMARY KEY AUTOINCREMENT, $colId INTEGER, $colTitle TEXT, '
        '$colDate TEXT)');
    await db.execute('CREATE TABLE $itemsTableOffline($colIdLocal INTEGER PRIMARY KEY AUTOINCREMENT, $colId INTEGER DEFAULT 0, $colTitle TEXT, '
        '$colDate TEXT)');
  }
  ...
  Future<List<Map<String, dynamic>>> getProductMapList() async {
    Database db = await this.database;
    var result = await db.query(itemsTable, orderBy: '$colTitle ASC');
    return result;
  }
  ...
  // // for offline -> online
  // Future<List<Map<String, dynamic>>> getProductMapListAddedOffline() async {
  //   Database db = await this.database;
  //   var result = await db.rawQuery("SELECT * from $itemsTable WHERE $colId = -1");
  //   return result;
  // }
  //
  // Future<List<Item>> getProductsAddedOffline() async {
  //   print("entered getProductsAddedOffline");
  //   var productMapList = await getProductMapListAddedOffline(); // Get 'Map List' from database
  //   int count = productMapList.length;         // Count the number of map entries in db table
  //
  //   List<Item> productList = List<Item>();
  //
  ...
  //   }
  //
  //   print("done getProductsAddedOffline");
  //
  //   return productList;
  // }
